...
Flutter LLM-агент с многоуровневой памятью, управлением контекстом, анализом пользователя и поддержкой MCP (Model Context Protocol). Поддерживает многооборотные диалоги с историей (Hive), стриминг ответов через SSE (OpenAI-совместимый API, прокси `openai.api.proxyapi.ru`), четыре стратегии контекста, три слоя памяти, function calling через MCP-серверы и тонкую настройку параметров запроса.
...
lib/
  ...
  services/
    api_service.dart                # SSE-стриминг, суммаризация, извлечение памяти/фактов, function calling
    ...
    mcp_service.dart                # MCP-клиент: подключение к серверам, вызов инструментов
  screens/
...
Приложение поддерживает подключение к удалённым MCP-серверам через Streamable HTTP транспорт.

### Архитектура

- **McpService** (`mcp_service.dart`) — управляет списком серверов, конфигурация сохраняется в Hive `settings` бокс (`mcp_servers` ключ)
- **McpServer** — подключение (JSON-RPC: `initialize` → `notifications/initialized` → `tools/list`), вызов инструментов (`tools/call`), отключение (HTTP DELETE)
- **McpTool** — описание инструмента (name, description, inputSchema), конвертация в OpenAI function calling формат

### Поток вызова инструментов (Function Calling)

1. При наличии подключённых MCP-серверов, список инструментов передаётся в API как `tools` (OpenAI формат)
2. Если модель возвращает `tool_calls` в SSE-ответе, они накапливаются в `ApiService.pendingToolCalls`
3. `ChatController._executeToolCallLoop()` выполняет инструменты через MCP и отправляет результаты обратно в API
4. Цикл повторяется пока модель вызывает инструменты; финальный текстовый ответ сохраняется как сообщение

### UI

- Настройки MCP-серверов доступны в модальном окне Settings (секция «MCP Servers»)
- Добавление/удаление серверов, подключение/отключение, просмотр доступных инструментов
- Баннер статуса при вызове инструментов (аналогично баннеру суммаризации)

### Пример: VkusVill MCP Server
...
Предоставляет 3 инструмента: поиск продуктов, детали продукта, создание ссылки на корзину.
